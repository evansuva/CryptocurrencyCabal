...
var amount = flag.Int("amount", -1, "The amount of BTC in satoshi.")
...
func getFundingParams(rawtx *blockChainInfoTx, vout uint32, amount int) (*wire.TxOut, *wire.TxOut, *wire.OutPoint) {
	...
	if amount > blkChnTxOut.Value {
		log.Fatal("Amount larger than balance.")
	}

	// Then convert it to a btcutil amount
	change := btcutil.Amount(int64(blkChnTxOut.Value - amount))
	...
	changeTxOut := wire.NewTxOut(int64(change), subscript)

	return changeTxOut, oldTxOut, outpoint
}
...
func main() {
	...
	change, oldTxOut, outpoint := getFundingParams(rawFundingTx, reqArgs.vout, reqArgs.amount)
	...
	txchange := createTxOut(change.Value, btcutil.Address(generateAddr(reqArgs.privKey.PubKey())))
	tx.AddTxOut(txout)
