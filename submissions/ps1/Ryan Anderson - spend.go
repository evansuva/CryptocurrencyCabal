...
** cs4501 Fall 2015
** Problem Set 1
** Modified by Ryan Anderson (rsa5bb) for PS1
*/
...
- An amount of Satoshi to transfer
...
var amount = flag.Int("amount", -1, "Amount of BTC to send.")
...
func main() {
	...
	if int64(*amount + TX_FEE) > oldTxOut.Value {
		//not enough to cover the transaction
		log.Fatal("Not enough BTC to cover the transaction.")
	}
	
	// Create the TxOut
	//txout := createTxOut(oldTxOut.Value, reqArgs.toAddress)
	txout := createTxOut(int64(*amount), reqArgs.toAddress)
	txout2 := createTxOut(int64(int(oldTxOut.Value) - *amount), generateAddr(reqArgs.privKey.PubKey()))
	tx.AddTxOut(txout)
	tx.AddTxOut(txout2)
