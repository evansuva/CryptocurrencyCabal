...
var amount = flag.Int("amount", -1, "The amount you would like to transfer")
...
func getArgs() requiredArgs {
	...
	if *toaddress == "" || *privkey == "" || *txid == "" || *vout == -1 || *amount == -1 {
		fmt.Println("\nThis program generates a bitcoin trans action that moves coins from an input to an output.\n" +
			...
			"of a tx), the index into the outputs of that tx that fund your\n" +
			"address, and the amount you are transacting. Use http://blockchain.info/pushtx to send the raw transaction.\n")
		flag.PrintDefaults()
		fmt.Println("")
		os.Exit(0)
	}
...
//LOOK UP THE AMOUNT OF BTC TRANSACTED-->don't have to change this
...
func getFundingParams(rawtx *blockChainInfoTx, vout uint32) (*wire.TxOut, *wire.OutPoint) {
	...
	// //netAmnt := *amount - TX_FEE

	// temp := int64(amnt)
	// totalCost := temp + TX_FEE

	// //"If the amount available in the specified transaction output (less the network fee) exceeds the amount to send, your program should print an error message.")

	// if int64(*amount) > int64(totalCost) {
	// 	fmt.Println(int64(*amount))
	// 	fmt.Println(int64(totalCost))
	// //	fmt.Println(netAmnt)
...
func main() {
	...
	addr := generateAddr((reqArgs.privKey.PubKey()))

	//change
	// Create the TxOut
	txout := createTxOut(oldTxOut.Value, reqArgs.toAddress)
	txoutret := createRetTxOut(oldTxOut.Value, addr)

	tx.AddTxOut(txout)
	tx.AddTxOut(txoutret)
...
func createRetTxOut(inCoin int64, addr btcutil.Address) *wire.TxOut {
	// Pay the minimum network fee so that nodes will broadcast the tx.
	temp := int64(*amount)
	outCoin := inCoin - temp - TX_FEE
	fmt.Println(inCoin)
	fmt.Println(temp)
	fmt.Println(outCoin)
	if(outCoin < 0) {
