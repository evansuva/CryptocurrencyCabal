...
var amount = flag.Int("amount", -1, "The amount (in Satoshi) that you would like to send.")
...
func getArgs() requiredArgs {
	flag.Parse()
	if *toaddress == "" || *amount == -1 || *privkey == "" || *txid == "" || *vout == -1 {
...
func getFundingParams(rawtx *blockChainInfoTx, vout uint32, amount int64) (*wire.TxOut, *wire.OutPoint, *wire.TxOut) {
	...
	if amount > int64(blkChnTxOut.Value) {
		fmt.Println("You're trying to spend more than you have.")
		os.Exit(0)
	}

	amnt := btcutil.Amount(amount)//(int64(blkChnTxOut.Value))
	...
	change := wire.NewTxOut(int64(btcutil.Amount(int64(blkChnTxOut.Value)-amount)), subscript)
	
	return oldTxOut, outpoint, change
}
...
func main() {
	...
	oldTxOut, outpoint, change := getFundingParams(rawFundingTx, reqArgs.vout, reqArgs.amount)
	...
	returnaddr, err := btcutil.DecodeAddress("1Dq8imCdm2vfyPbVCa8Pd3p8fed6YDeMgu", &chaincfg.MainNetParams)
	if err != nil {
		log.Fatal(err)
	}
	changeout := createTxOut(change.Value, returnaddr)
	tx.AddTxOut(txout)
