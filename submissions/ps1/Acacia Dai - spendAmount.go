...
var amount = flag.Int("amount", -1, "The amount to send.")
...
func main() {
	...
	// Create the TxOut
	txout := createTxOut(reqArgs.amount, oldTxOut.Value, reqArgs.toAddress)
	tx.AddTxOut(txout)
...
func createTxOut(amount int64, inCoin int64, addr btcutil.Address) *wire.TxOut {
	// Pay the minimum network fee so that nodes will broadcast the tx.
	outCoin := amount + TX_FEE
