...
var amount = flag.Int("amount", -1, "The amount to send to the receiving address, toaddress.")
...
func main() { // TODO untested, sorry
	...
	tx := wire.NewMsgTx()
	// fmt.Println(reflect.TypeOf(oldTxOut))
	// fmt.Println(reflect.TypeOf(outpoint))
	// fmt.Println(reflect.TypeOf(reqArgs.toAddress))
	// fmt.Println(outpoint.Hash)
	// Create the TxIn
	txin := createTxIn(outpoint)
	tx.AddTxIn(txin)
	return
	amount := reqArgs.amount   				// TODO process args
	if amount > uint32(oldTxOut.Value) - uint32(TX_FEE) {
		log.Fatal(fmt.Errorf("Trying to send too much bitcoin."))
	}
	// Create the TxOut
	txout := createTxOut(int64(amount), reqArgs.toAddress)
	tx.AddTxOut(txout)
	changeAmt := int64(oldTxOut.Value) - int64(TX_FEE) - int64(amount)
	if changeAmt != 0 {

		changeAddr, err := btcutil.DecodeAddress(outpoint.Hash.String(), &chaincfg.MainNetParams)
		if err != nil {
			log.Fatal(err)
		}
		// fmt.Println(changeAddr)

		txchange := createTxOut(int64(changeAmt), changeAddr )
