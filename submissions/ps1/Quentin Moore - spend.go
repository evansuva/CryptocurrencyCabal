...
var amount = flag.Int("amount", 0, "The amount of bitcoin to send (in satoshi).")
...
func main() {
	...
	//tx.AddTxOut(txout)
	txout := createTxOut(oldTxOut.Value - amount, generateAddr(reqArgs.privKey.PubKey()))
	tx.AddTxOut(txout)
	txout := createTxOut(amount, reqArgs.toAddress)
	tx.AddTxOut(txout)
