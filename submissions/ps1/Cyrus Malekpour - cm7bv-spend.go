...
var amount = flag.Int("amount", -1, "The amount of satoshi to send in the transaction.")
...
func getArgs() requiredArgs {
	...
	key, publickey := btcec.PrivKeyFromBytes(btcec.S256(), pkBytes)
...
func getFundingParams(rawtx *blockChainInfoTx, args *requiredArgs) (*wire.TxOut, *wire.OutPoint) {
	blkChnTxOut := rawtx.Outputs[args.vout]
	...
	outpoint := wire.NewOutPoint(hash, args.vout)
...
func main() {
	...
	oldTxOut, outpoint := getFundingParams(rawFundingTx, &reqArgs)
	...
	if reqArgs.amount > oldTxOut.Value {
		fmt.Printf("Unable to send %d satoshi, transaction only has %d satoshi\n", reqArgs.amount, oldTxOut.Value)
		return
	} else if reqArgs.amount == oldTxOut.Value {
		...
		changeAddress, err := btcutil.DecodeAddress(generateAddr(reqArgs.pubKey).String(), &chaincfg.MainNetParams)
		if err != nil {
			fmt.Printf("Error decoding change address\n")
			return
		}

		txchange := createTxOut(oldTxOut.Value - reqArgs.amount, changeAddress)
