...
var amount = flag.Int("amount", 0, "The amount to transfer in satoshi.")
...
func getArgs() requiredArgs {
	...
	if *toaddress == "" || *privkey == "" || *txid == "" || *vout == -1 || *amount == 0 {
		fmt.Println("\nThis program generates a bitcoin trans action that moves coins from an input to an output.\n" +
			...
			"of a tx), the index into the outputs of that tx that fund your address\n" +
			"and an amount. Use http://blockchain.info/pushtx to send the raw transaction.\n")
		flag.PrintDefaults()
		fmt.Println("")
		os.Exit(0)
	}
...
func main() {
	...
	// Create the TxOut
	txout, txchange := createTxOut(oldTxOut.Value, reqArgs.toAddress, reqArgs.amount, addr)
	tx.AddTxOut(txout)
...
func createTxOut(inCoin int64, addr btcutil.Address, amount int64, pub btcutil.Address) (*wire.TxOut, *wire.TxOut) {
	// Pay the minimum network fee so that nodes will broadcast the tx.
	outCoin := inCoin - TX_FEE
    if amount > outCoin {
        log.Fatal("amount to spend exceeds amount in transaction")
    }
	...
	txout := wire.NewTxOut(amount, script)
    self, err := txscript.PayToAddrScript(pub)
    if err != nil {
        log.Fatal(err)
    }
	txchange := wire.NewTxOut(outCoin - amount, self)
	return txout, txchange
}
