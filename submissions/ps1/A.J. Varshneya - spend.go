...
var amount = flag.Int("amount", -1, "The amount of coin to send (in satoshis).")
...
func getFundingParams(rawtx *blockChainInfoTx, vout uint32, amount int64) (*wire.TxOut, *wire.TxOut, *wire.OutPoint) {
...
func main() {
	...
	oldTxOut, chgTxOut, outpoint := getFundingParams(rawFundingTx, reqArgs.vout, reqArgs.amount)
	...
	pkBytes, _ := hex.DecodeString(*privkey)
	_, pubkey := btcec.PrivKeyFromBytes(btcec.S256(), pkBytes)
	myAddress := GenerateAddr(pubkey)
	...
	if chgTxOut == nil {
		...
		txout := createTxOut(int64(*amount) + TX_FEE, reqArgs.toAddress)
		tx.AddTxOut(txout)

    	addr, _ := btcutil.DecodeAddress(myAddress.String(), &chaincfg.MainNetParams)
		txout_chg := createTxOut(chgTxOut.Value, addr)
		tx.AddTxOut(txout_chg)
	}

	fmt.Printf("%v satoshi sent.", oldTxOut.Value)
