...
var amount = flag.Int("amount", -1, "The amount to send from the input address")
...
func main() {
	...
	//Compare the inputs with the outputs
	if(oldTxOut.Value - TX_FEE - reqArgs.amount < 0){
		//var lol int64 = oldTxOut.Value - TX_FEE - reqArgs.amount
		fmt.Printf("The amount you specified is larger than the inputs to the given from address. \n")
	} else {

		inputAddress := generateAddr(reqArgs.privKey.PubKey())
		...
		//Create a transaction back to the input address
		//This is only done if there is any bitcoin left over to send back
		if((oldTxOut.Value - reqArgs.amount  - TX_FEE) != 0){
			txback := createTxBack((oldTxOut.Value - reqArgs.amount  - TX_FEE), inputAddress)
			tx.AddTxOut(txback)	
		} 
...
//Specifically it creates a transaction back with extra to input address
func createTxBack(inCoin int64, addr btcutil.Address) *wire.TxOut {
