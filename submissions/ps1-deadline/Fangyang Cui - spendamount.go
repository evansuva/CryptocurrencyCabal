...
var amount = flag.Int("amount", 0, "The amount in satoshis to transfer")
...
func getFundingParams(rawtx *blockChainInfoTx, vout uint32, amount uint32) (*wire.TxOut, *wire.OutPoint) {
	...
	available := btcutil.Amount(int64(blkChnTxOut.Value))
	amnt := btcutil.Amount(amount)
	
	if amnt > available || amnt <= TX_FEE{
		log.Fatal("Send amount greater than available balance or not enough to pay network fee")
	}
