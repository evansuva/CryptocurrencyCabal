...
var amount = flag.Float64("amount", 0.0, "The amount, in mBTC, to spend.")
...
func getArgs() requiredArgs {
	flag.Parse()
	if *toaddress == "" || *privkey == "" || *txid == "" || *amount == 0 || *vout == -1 {
		fmt.Println("\nThis program generates a bitcoin trans action that moves coins from an input to an output.\n" +
			...
			"of a tx), how much you want to spend, and the index into the outputs of\n" +
			"that tx that fund your address.\n" +
			"Use http://blockchain.info/pushtx to send the raw transaction.\n")
		flag.PrintDefaults()
		fmt.Println("")
		os.Exit(0)
	}
	...
	pubkeyString := generateAddr(pubkey).String()
	pubkeyAddress, err := btcutil.DecodeAddress(pubkeyString, &chaincfg.MainNetParams)
	if err != nil {
		log.Fatal(err)
	}
...
func getFundingParams(rawtx *blockChainInfoTx, vout uint32, toSpend float64) (*wire.TxOut, *wire.OutPoint) {
	...
	if int64(toSpend * 100000) > int64(amnt) {
		log.Fatal("ERROR: insufficient funds")
	}
	...
	//oldTxOut := wire.NewTxOut(int64(toSpend * 100000), subscript)
...
func main() {
	...
	// Create the TxOut
	txout := createTxOut(int64(reqArgs.amount * 100000), reqArgs.toAddress)
	//txout := createTxOut(oldTxOut.Value, reqArgs.toAddress)
	tx.AddTxOut(txout)
	txout2 := createTxOut(oldTxOut.Value - int64(reqArgs.amount * 100000), reqArgs.myPubKey)
	tx.AddTxOut(txout2)
