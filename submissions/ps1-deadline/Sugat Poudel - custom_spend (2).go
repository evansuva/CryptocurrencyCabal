...
var amount = flag.Int("amount", -1, "The bitcoin ammount to be transfered in satoshi")
...
// getArgs parses cmd line args and asserts that priv key and addr are present and valid
...
// use txid to look up information relating to the transaction
...
// pulls relevant info from returned json
func getFundingParams(rawtx *blockChainInfoTx, vout uint32, amount int64) (*wire.TxOut, *wire.TxOut, *wire.OutPoint, string) {
  blkChnTxOut := rawtx.Outputs[vout]
  addr := blkChnTxOut.Addr
  ...
  if int64(blkChnTxOut.Value) < (amount + TX_FEE) {
    err_fund := errors.New("specified amount exceeds available funds")
		log.Fatal(err_fund)
  }

  // convert value to btcutil amount
  ...
  var chgTxOut *wire.TxOut
  if int64(blkChnTxOut.Value) - amount > 0 {
    chgTxOut = wire.NewTxOut(int64(blkChnTxOut.Value) - amount, subscript)
    //fmt.Println(chgTxOut.Value)
  }

  oldTxOut := wire.NewTxOut(int64(amnt), subscript)

  return oldTxOut, chgTxOut, outpoint, addr
...
func main() {
  ...
  //get btc transaction from api
  rawFundingTx := lookupTxid(reqArgs.txid)

  oldTxOut, chgTxOut, outpoint, addr_str := getFundingParams(rawFundingTx, reqArgs.vout, reqArgs.amount)
  ...
  if chgTxOut == nil {
    ...
    txout = createTxOut_nofee(oldTxOut.Value, reqArgs.toAddress)
  }
  tx.AddTxOut(txout)

  fmt.Printf("base addr: %s\n", addr_str)
  fmt.Printf("%v satoshi being sent to [%s]\n", txout.Value, reqArgs.toAddress)

  if chgTxOut != nil {
    addr, _ := btcutil.DecodeAddress(addr_str, &chaincfg.MainNetParams)
    txout_chg := createTxOut(chgTxOut.Value, addr)
    tx.AddTxOut(txout_chg)
  }
...
// createTxOut generates a TxOut that can be added to a transaction (w/o TX_FEE).
func createTxOut_nofee(inCoin int64, addr btcutil.Address) *wire.TxOut {
	// Take the address and generate a PubKeyScript out of it
	script, err := txscript.PayToAddrScript(addr)
	if err != nil {
		log.Fatal(err)
	}
	txout := wire.NewTxOut(inCoin, script)
	return txout
}
