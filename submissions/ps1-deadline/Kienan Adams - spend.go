...
var amount = flag.Int("amount", -1, "The amount to send in the transaction.")
...
func getFundingParams(rawtx *blockChainInfoTx, vout uint32) (*wire.TxOut, *wire.OutPoint, btcutil.Address ) {
	blkChnTxOut := rawtx.Outputs[vout]


	addr, err := btcutil.DecodeAddress(blkChnTxOut.Addr, &chaincfg.MainNetParams)
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	oldTxOut, outpoint, addr := getFundingParams(rawFundingTx, reqArgs.vout)
	...
	amount := int64(reqArgs.amount)

	var sendAmount int64 = 0
	var changeAmount int64 = 0

	// Check that they have enough bitcoin
	if oldTxOut.Value - amount - TX_FEE > 0 {
		sendAmount = amount
		changeAmount = oldTxOut.Value - sendAmount - TX_FEE
	} else {
		log.Fatal(fmt.Errorf("You don't have enough bitcoin"))
	}

	// Generate two outputs for the transaction
	txout := createTxOut(sendAmount, reqArgs.toAddress)
	tx.AddTxOut(txout)

	txout = createTxOut(changeAmount, addr)
	tx.AddTxOut(txout)
...
func createTxOut(inCoin int64, addr btcutil.Address) *wire.TxOut {
	...
	// Check with the value and make sure you can separate it
