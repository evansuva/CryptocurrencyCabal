...
var amount = flag.Int("amount", -1, "Amount of money you want to transfer")
...
func main() {
	...
	// Create the TxOut
	txout := createTxOut(oldTxOut.Value, reqArgs.toAddress, reqArgs.amount)
	tx.AddTxOut(txout)
...
func createTxOut(inCoin int64, addr btcutil.Address, amnt int64) *wire.TxOut {
	// Pay the minimum network fee so that nodes will broadcast the tx.
	outCoin := inCoin - TX_FEE
	if amnt > outCoin {
		log.Fatal(fmt.Errorf("Amount specified exceeds maximum allowed"))
	}
	...
	txout := wire.NewTxOut(amnt, script)
	return txout
}
